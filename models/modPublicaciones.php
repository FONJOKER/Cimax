...
class modPublicaciones{
    ...
    public static function mdlMostrarRegistrosPaginados($pagina_actual,$registros_por_pagina,$consulta){
        
        $sentenciaMYSQL=$consulta;
    ...
    public static function mdlInsertarConImagen($tabla,$datos)
    {
        ...
        if($_FILES["foto"]["size"]>0)
        {
            ...
            else
            {
                ...
                modPublicaciones::mdlInsertarRegistros($tabla,$datos);
    ...
    public static function mdlBorrarRegistrosConImagen($tabla,$campo_filtrar,$valores_eliminar){
        ...
        //---- ELIMINAR IMAGENES----------------------------------
        $registros=modPublicaciones::mdlMostrarRegistros($tabla,null,null);
        foreach($valores_eliminar as $id)// $valores_eliminar es un array escalar con los ids de las fotos a eliminar
        {
            foreach($registros as $registro)
            {
                if($registro["id"]==$id)
                {
                    if($registro["imagen"]!="views/img/publicaciones/default.jpg")
                    {
