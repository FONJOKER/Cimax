...
class FuncionesController{
    ...
    public static function ctrCompruebaRegistro()
    {
        ...
        $funciones = new FuncionesController; 
            ...
            if(isset($_POST["id_redireccion"]))
            {
                echo "<script>
                    window.location='index.php?ruta=login&id_redireccion=".$_POST["id_redireccion"]."';
                </script>";
            }
            else if(isset($_POST["id_redireccion_publicacion"]))
            {
                echo "<script>
                    window.location='index.php?ruta=login&id_redireccion_publicacion=".$_POST["id_redireccion_publicacion"]."&#formularioInsertarComentario';
                </script>";
            }
            else
            {
            ...
            // $usuario = $usuarios_model->ctrMostrarRegistro("email",$_POST["email"]);
            // $_SESSION["usuario"]=$usuario;
            // $_SESSION["notificaciones"] = "pendiente";
        ...
        if($usuario != false)
        {
            ...
            if($usuario["pass"]==$funciones->ctrCifrar($_POST["pass"]))
            {
                ...
                if($usuario["tipo"] != "administrador")
                {
                    ...
                    {
                        echo "<script>
                            window.location='index.php?ruta=servicio-detalle&id=".$_POST["id_redireccion"]."';
                        </script>";
                    }
                    else if(isset($_POST["id_redireccion_publicacion"]))
                    {
                        echo "<script>
                            window.location='index.php?ruta=blog-detalle&id=".$_POST["id_redireccion_publicacion"]."&#formularioInsertarComentario';
                        </script>";
                    }
                    else
                    {
